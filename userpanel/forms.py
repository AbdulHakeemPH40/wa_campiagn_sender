...
from sitevisitor.models import CustomUser, Profile, FreeTrialPhone
...
class WhatsAppNumberForm(forms.Form):
    ...
    def clean_whatsapp_number(self):
        ...
        if not re.match(r'^\+?[0-9]+$', number):
            raise forms.ValidationError("WhatsApp number can only contain numbers and the + sign")
        if '+' in number and number.index('+') > 0:
            raise forms.ValidationError("The + sign can only be at the beginning of the number")
        return number
