...
from .forms import LoginForm, SignupForm, CustomPasswordResetForm, CustomSetPasswordForm, OTPVerificationForm, NewsletterForm, ContactForm, WhatsAppNumberForm
from .models import CustomUser, Profile, EmailVerification, PasswordReset, OTPVerification, NewsletterSubscriber, WhatsAppNumber, ContactMessage
...
def IndexView(request):
    ...
    plans = SubscriptionPlan.objects.filter(is_active=True).order_by('price') # Order by price for consistent display
    context = {
        ...
        if form.is_valid():
            ...
            try:
                ...
                if 'quota exceeded' in str(e).lower():
                    return redirect('sitevisitor:error_email') + '?type=quota_exceeded'
                elif 'unauthorized' in str(e).lower():
                    return redirect('sitevisitor:error_email') + '?type=invalid_api_key'
                else:
                    return redirect('sitevisitor:error_email') + '?type=general'
...
ALL_BLOG_POSTS = [
    {
        'title': 'Direct WhatsApp Outreach: A Game-Changer',
        'url_name': 'blog_post_direct_outreach',
        'slug': 'direct-whatsapp-outreach-browser-extensions',
        'image': 'image/post_direct_outreach.png',
        'description': 'Discover how browser extensions simplify WhatsApp marketing, offering a direct, no-fuss approach compared to complex APIs or spreadsheet dependencies...'
    },
    {
        'title': 'Sending Bulk WhatsApp Messages Safely',
        'url_name': 'blog_post_safe_sending',
        'slug': 'sending-bulk-whatsapp-safely',
        'image': 'image/blog_safe_sending.png',
    ...
    },
    {
        ...
        'image': 'image/post_personalization.png',
    ...
    },
    {
        ...
        'image': 'image/post_contact_management.png',
    ...
    },
    {
        ...
        'image': 'image/blog_event_marketing.png',
    ...
    },
    {
        ...
        'image': 'image/blog_campaign_checklist.png',
    ...
    },
    {
        ...
        'image': 'image/blog_advanced_safety.png',
    ...
    {
        'title': 'More Than Just WhatsApp Web: Extension Power',
        'url_name': 'blog_post_extension_power',
        'slug': 'extension-power-more-than-whatsapp-web',
        'image': 'image/blog_extension_power.png',
        'description': 'Understand the limitations of WhatsApp Web and how a dedicated browser extension provides superior features for business outreach.'
    },
    {
        'title': 'Maximize Your Reach: Timing & Frequency',
        'url_name': 'blog_post_timing_frequency',
        'slug': 'timing-frequency-maximizing-reach',
        'image': 'image/blog_timing_frequency.png',
...
def blog_post_direct_outreach(request):
    context = {
        'posts': ALL_BLOG_POSTS,
        'current_slug': 'direct-whatsapp-outreach-browser-extensions'
    }
...
def blog_post_extension_power(request):
    context = {
        'posts': ALL_BLOG_POSTS,
        'current_slug': 'extension-power-more-than-whatsapp-web'
    }
...
def _send_verification_email_util(request, user, token):
    try:
        ...
        if 'quota exceeded' in str(e).lower():
            ...
            return redirect('sitevisitor:error_email') + '?type=rate_limit'
        else:
            return redirect('sitevisitor:error_email') + '?type=general'
    except Exception as e:
        # Handle other email errors
...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get WhatsApp number from form and create a WhatsAppNumber object
            whatsapp_number_str = form.cleaned_data.get('phone', '')
            if whatsapp_number_str:
                try:
                    # Ensure the user has a profile, create if not
                    profile, created = Profile.objects.get_or_create(user=user)
                    WhatsAppNumber.objects.create(profile=profile, number=whatsapp_number_str, is_primary=True, is_verified=False)
                except IntegrityError:
                    # Handle case where WhatsApp number already exists
                    user.delete()  # Clean up the user that was just created
...
def download_view(request):
    """Extension download page with installation guide"""
    return render(request, 'sitevisitor/download.html')

def how_to_use_view(request):
    """Complete step-by-step guide on how to use WA Campaign Sender"""
    return render(request, 'sitevisitor/how_to_use.html')
...
def robots_txt_view(request):
    ...
    Controls crawling for WA Campaign Sender website.
    """
    from django.http import HttpResponse
    
    content = "\n".join([
        "# Robots.txt for wacampaignsender.com - WhatsApp Marketing Tool",
        "# Controls search engine crawlers for proper indexing and security.",
        "",
        ...
        "Crawl-delay: 1",
        "",
        "# Sitemap for WA Campaign Sender",
...
def complete_social_signup(request):
    """Handle collecting WhatsApp number and full name from users who sign up with Google"""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Validate the data
            if not whatsapp_number or not full_name_form:
                messages.error(request, 'Please provide both your full name and WhatsApp number.')
            ...
            return redirect('/social-auth/complete/google-oauth2/')
    else:
        form = WhatsAppNumberForm(initial={'full_name': full_name})
